...
//My own grid cell index computation macro
#define toGridCellIndex(gridCell) ((gridCell.x + gridCell.y * gridResolution + gridCell.z * gridResolution * gridResolution))
...
#if !ZIP_ITERATOR
  cudaMalloc(&dev_pos2, N * sizeof(glm::vec3));
...
__global__ void kernComputeIndices(int N, int gridResolution,
  ...
  glm::vec3 *pos, int *indices, int *gridIndices) {
    ...
    if (idx >= N) return;
    glm::ivec3 gridCell = glm::floor((pos[idx] - gridMin) * inverseCellWidth);

    gridIndices[idx] = toGridCellIndex(gridCell);
    indices[idx] = idx;
}
...
__global__ void kernIdentifyCellStartEnd(int N, int *particleGridIndices,
  ...
  // "this index doesn't match the one before it, must be a new cell!"
    ...
    if (idx == N - 1) {
        ...
        int nextGridIndex = particleGridIndices[idx + 1];
        if (selfGridIndex != nextGridIndex) {
            gridCellEndIndices[selfGridIndex] = idx;
            gridCellStartIndices[nextGridIndex] = idx + 1;
        }
    }
}
        ...
        for (int j = glm::max(lo.y, 0); j <= glm::min(hi.y, gridResolution - 1); ++j) {
            for (int k = glm::max(lo.z, 0); k <= glm::min(hi.z, gridResolution - 1); ++k) {
                ...
                for (int l = gridCellStartIndices[gridIndex3Dto1D(i, j, k, gridResolution)]; l <= gridCellEndIndices[gridIndex3Dto1D(i, j, k, gridResolution)]; ++l) {
                    ...
                    int pIdx = particleArrayIndices[l];
...
void Boids::stepSimulationScatteredGrid(float dt) {
  ...
  // - Ping-pong buffers as needed
    ...
    /*__global__ void kernComputeIndices(int N, int gridResolution,
        glm::vec3 gridMin, float inverseCellWidth,
        glm::vec3 * pos, int *indices, int *gridIndices)*/
    kernComputeIndices << <blocks, threadsPerBlock >> > (numObjects, gridSideCount, gridMinimum, gridInverseCellWidth,
    ...
    dim3 resetBlocks = dim3(divup(gridCellCount, blockSize));
    kernResetIntBuffer << <resetBlocks, threadsPerBlock >> > (gridCellCount, dev_gridCellStartIndices, -1);
    kernResetIntBuffer << <resetBlocks, threadsPerBlock >> > (gridCellCount, dev_gridCellEndIndices, -1);

    kernIdentifyCellStartEnd << <blocks, blockSize >> > (numObjects, dev_particleGridIndices,
        dev_gridCellStartIndices, dev_gridCellEndIndices);

    kernUpdateVelNeighborSearchScattered << <blocks, threadsPerBlock >> > (numObjects, gridSideCount, gridMinimum,
...
__global__ void kernShuffle(int N, int *particleArrayIndices, glm::vec3 *posTo, glm::vec3 *posFrom, glm::vec3 *velTo, glm::vec3 *velFrom) {
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
...
void Boids::stepSimulationCoherentGrid(float dt) {
  ...
  // - Ping-pong buffers as needed. THIS MAY BE DIFFERENT FROM BEFORE.
    ...
    glm::vec3 *temp;
#if ZIP_ITERATOR
    auto dev_thrustValuesBegin = thrust::make_zip_iterator(thrust::make_tuple(dev_thrust_vel1, dev_thrust_pos));
    thrust::sort_by_key(dev_thrust_keys, dev_thrust_keys + numObjects, dev_thrustValuesBegin);
#endif
#if !ZIP_ITERATOR
    thrust::device_ptr<int> dev_thrust_values(dev_particleArrayIndices);
    thrust::sort_by_key(dev_thrust_keys, dev_thrust_keys + numObjects, dev_thrust_values);
